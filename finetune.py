...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='./models/baseline_best.pth', help="restore checkpoint")
    ...
    # create log txt
    log_txt_file = os.path.join(log_root, project_name + '_log.txt')    #./logs/project_name_log.txt
    custom_print(project_name, log_txt_file, 'w')

    # create model save_dir
    models_root = './models'
    ...
    for p in net.sp2[0].parameters():
        p.requires_grad = False
    for p in net.cls[0].parameters():  # freeze cls parameters since no cls label in video dataset
        p.requires_grad = False
    ...
    net.load_state_dict(state_dict) # load weights

    net.train()

    if use_flow == False:
        train_finetune(net, train_datapath, device, batch_size, log_txt_file, val_datapath, models_train_best,
                       models_train_last, lr, lr_de, epochs, log_interval, val_interval)
    else:
        train_finetune_with_flow(net, train_datapath, device, batch_size, log_txt_file, val_datapath, models_train_best,
                                 models_train_last, lr, lr_de, epochs, log_interval, val_interval)
