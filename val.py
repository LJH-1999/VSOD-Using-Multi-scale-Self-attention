    ...
    with torch.no_grad():
        ...
        for p in range(len(datapath)):
            ...
            for s in range(len(all_class)):
                ...
                image_list.append(
                    list(map(lambda x: os.path.join(datapath[p], img_dir_name, all_class[s], x), image_path)))
                gt_list.append(list(map(lambda x: os.path.join(datapath[p], gt_dir_name, all_class[s],
                                                               x.replace(img_ext[p], gt_ext[p])), image_path)))
            for i in range(len(image_list)):
                ...
                for q in range(cur_class_mask.size(0)):
                    single_p, single_j = calc_precision_and_jaccard(cur_class_mask[q, :, :].numpy(),
                                                                    cur_class_gt[q, :, :].numpy())
...
def validation_with_flow(net, datapath, device, group_size=5, img_size=224, img_dir_name='image',
    ...
    with torch.no_grad():
        ...
        for p in range(len(datapath)):
            ...
            for s in range(len(all_class)):
                ...
                image_list.append(
                    sorted(list(map(lambda x: os.path.join(datapath[p], img_dir_name, all_class[s], x), image_path))))
                flow_list.append(sorted(list(
                    map(lambda x: os.path.join(datapath[p], 'flow', all_class[s], x.replace(img_ext[p], '.jpg')),
                        flow_path))))
                gt_list.append(sorted(list(map(lambda x: os.path.join(datapath[p], gt_dir_name, all_class[s],
                                                                      x.replace(img_ext[p], gt_ext[p])), image_path))))
