    ...
    with torch.no_grad():
        for p in range(len(datapath)):
            ...
            for s in range(len(all_class)):
                ...
                save_list.append(
                    list(map(lambda x: os.path.join(save_root_path[p], all_class[s], x[:-4] + '.png'), image_path)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='./models/baseline_best_ft.pth', help="restore checkpoint")
    parser.add_argument('--data_path', default='./vsod_testset/ViSal', help="dataset for evaluation")
    parser.add_argument('--output_dir', default='/home/jiahao/PycharmProjects/eval-co-sod/compare_/pred/baseline/ViSal', help='directory for result')
    parser.add_argument('--task', default='VSOD', choices=['CoS_CoSD', 'VSOD'], help='task')
