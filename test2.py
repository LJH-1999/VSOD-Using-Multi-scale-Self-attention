    ...
    def __init__(self, dim, depth, heads, mlp_dim, group=5, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Residual(PreNorm(dim, Attention(dim, num_heads=4, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=2))),
...
f = nn.Conv3d(512, 512, kernel_size=(1, 2, 2), padding=(2, 0, 0), stride=2)
x = torch.ones(size=(8, 512, 5, 28, 28))
x = f(x)
print(x.shape)

attention = Attention(dim=512, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=2)
x = attention.forward(x=torch.randn(size=(8,5*28*28,512)), num_frame=5, H=28, W=28)
print(x.shape)
