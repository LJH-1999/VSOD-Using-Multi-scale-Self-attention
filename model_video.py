...
def vgg(cfg, i=3, batch_norm = True):
    ...
    for v in cfg:
        if v == 'M': layers += [nn.MaxPool2d(kernel_size=2, stride=2)]
            ...
            conv2d = nn.Conv2d(in_channels, v, kernelsize=3, padding=1)
            if batch_norm:
...
def concat_r():
    ...
    layers += [nn.Conv2d(512, 512, 1, 1)]
    layers += [nn.ReLU()]
    layers += [nn.Conv2d(512, 512, 3, 1, 1)] # 3 1 1 don't change shape
    layers += [nn.ReLU()]
...
def fuse_hsp(x, p, group_size = 5):
    ...
    for i in range(x.size(0)):
        tmp = x[i, :]               # element with index = i in first dimension
        if i == 0:
            nx = tmp.expand_as(t)   # make tmp the shape as t
        else:
